...
OMT better-cut 经 :meth:`add_hard` 并入断言后，经 :class:`RootForcedTracker`
在独立 Context 上**增量**刷新根级 ``consequences`` 强制赋值（``_root_fixed``），
在 refocus 建图时与搜索 trail 合并，跨 cut 投影子句 / 剪候选；consequences 的
rlimit 累加到 ``consequence_rlimit``，不污染主 Solver 搜索口径。
"""
...
class PolicyDecider:
    ...
    def add_hard(self, *exprs) -> None:
        ...
        供后续建图投影；不调用 z3 理论引擎以外的预处理，避免改写与 propagator
        注册原子不一致。消耗累加到 ``consequence_rlimit``。
        """
        if not exprs:
            return
